...
> **구현 상태**: 현재 구현된 컴포넌트만 표시합니다.

### 6.1 버튼 ✅ 구현됨

**구현된 변형**:
- `primary`: 주요 액션 (브랜드 컬러)
- `secondary`: 보조 액션 (보더 스타일)
- `ghost`: 텍스트 버튼
- `danger`: 삭제/경고 액션 (빨간색)
...
**구현된 기능**:
- `label` prop: 레이블 자동 표시
- `error` prop: 에러 메시지 및 스타일링
- 포커스 링 (브랜드 컬러)
- 호버/포커스 상태 애니메이션
...
> **Note**: 현재 lucide-react는 설치되어 있지만, 대부분의 UI에서는 이모지 또는 SVG를 사용 중입니다.

**권장 아이콘 매핑** (구현 시 참고):

| 기능 | 아이콘 | 사용 위치 |
|------|--------|----------|
| IP 변경 | `Zap` | 프로젝트 생성 모드 |
| 스케치 실사화 | `Pencil` | 프로젝트 생성 모드 |
| 히스토리 | `Library` | 히스토리 페이지 |
| 설정 | `Settings` | 사용자 설정 |
| 업로드 | `Upload` | 이미지 업로더 |
| 다운로드 | `Download` | 결과 다운로드 |
| 삭제 | `Trash2` | 삭제 버튼 |
| 편집 | `Edit3` | 수정 버튼 |
| 저장 | `Save` | 저장 버튼 |
| 새로고침 | `RefreshCw` | 재생성 |
| 닫기 | `X` | 모달/토스트 닫기 |
| 로딩 | `Loader2` | 로딩 스피너 |
| 성공 | `CheckCircle` | 완료 상태 |
| 에러 | `AlertCircle` | 에러 상태 |
| 사용자 | `User` | 프로필 |
| 폴더 | `FolderOpen` | 프로젝트 |
| 이미지 | `Image` | 갤러리 |
| 플러스 | `Plus` | 추가 버튼 |
...
| 컴포넌트 | 구현 상태 | 우선순위 | 파일 위치 |
|----------|----------|----------|-----------|
| Button (4가지 변형) | ✅ 완료 | P0 | `components/ui/button.tsx` |
| Input (label, error) | ✅ 완료 | P0 | `components/ui/input.tsx` |
| ImageUploader (드래그앤드롭) | ✅ 완료 | P0 | `components/ui/image-uploader.tsx` |
| Spinner (로딩) | ✅ 완료 | P0 | `button.tsx` 내장 |
| Card | 🚧 예정 | P0 | - |
| Modal | 🚧 예정 | P0 | - |
| Toast | 🚧 예정 | P0 | - |
| Progress Bar | 🚧 예정 | P0 | - |
