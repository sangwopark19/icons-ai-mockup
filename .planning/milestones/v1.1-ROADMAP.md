...
**Status:** SHIPPED 2026-05-04 with accepted deferred evidence
**Phases:** 7-13
**Total Plans:** 23

## Overview

v1.1 added OpenAI GPT Image 2 beside the existing Gemini provider. It shipped provider-aware schema, queue routing, admin key separation, OpenAI IP Change v2, OpenAI Sketch to Real v2, same-provider result continuation, OpenAI partial edit/style-copy paths, and admin retry recovery. The milestone closed with accepted deferred evidence for product model-label display and live/browser proof gaps.
...
- ✅ **v1.1 OpenAI GPT Image 2 Dual Provider** — Phases 7-13 shipped 2026-05-04 with accepted deferred evidence, started 2026-04-24
  ...
  - Integration check: [v1.1-INTEGRATION-CHECK.md](./v1.1-INTEGRATION-CHECK.md)
...
- [x] **Phase 8: OpenAI IP Change Parity** — completed with live/browser evidence deferred
...
- [x] **Phase 9: OpenAI Sketch to Real Parity** — completed with `OSR-03` transparent evidence exception-scoped
...
- Gemini and OpenAI remain parallel providers rather than a migration.
- Provider/model lineage is stored as first-class generation metadata for routing, support, history, and continuation.
- OpenAI transparent-background output is handled through opaque-first generation plus local background-removal post-processing.
- Product UI currently uses product-safe `v1`/`v2` labels; the stricter `PROV-03` model-display requirement remains accepted deferred work.

**Issues Resolved:**

- Provider-aware schema, queue payloads, worker dispatch, and admin API key separation.
- OpenAI IP Change and Sketch v2 source/UI/result lifecycle.
- OpenAI partial edit, style-copy, regenerate, and admin retry continuation metadata.
- Missing Phase 9 verification/orphaned requirements and stale Phase 8 transparent-IP wording.

**Issues Deferred:**

- `PROV-03` explicit model label in product result/history UI.
- `OSR-03` final live transparent PNG alpha/composite evidence.
- Phase 8 and Phase 10 authenticated live OpenAI/browser UAT evidence.

**Technical Debt Incurred:**

- Several phase validation artifacts remain `human_needed` or partial under strict Nyquist audit rules.
- Phase 7 residual review risks remain around uploaded image path ownership, active API key partial uniqueness, and client-safe schema separation.
