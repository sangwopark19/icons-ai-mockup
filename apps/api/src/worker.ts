...
const generationWorker = new Worker<GenerationJobData>(
  ...
  async (job: Job<GenerationJobData>) => {
    ...
    try {
      ...
      // ============================================================================
      // 스타일 복사 모드: 전용 함수 사용 (v3 개선)
      // ============================================================================
      if (parentGenerationId && options.styleCopy) {
        console.log(`🎨 스타일 복사 모드: 부모 Generation ${parentGenerationId}에서 스타일 복사`);

        if (!sourceImageBase64 || !characterImageBase64) {
          throw new Error('스타일 복사에는 원본 이미지와 캐릭터 이미지가 필요합니다');
        }

        // 부모 Generation의 선택된 이미지 로드 (스타일 참조용)
        ...
        try {
          ...
          if (parentGeneration?.images[0]) {
            ...
            styleReferenceBase64 = parentImageBuffer.toString('base64');
            console.log(`✅ 스타일 참조 이미지 로드 완료: ${parentImagePath}`);
          } else {
            console.warn(`⚠️ 부모 Generation에 선택된 이미지가 없습니다: ${parentGenerationId}`);
          }
        } catch (error) {
          console.error(`❌ 스타일 참조 이미지 로드 실패:`, error);
        }

        // 스타일 참조 이미지가 있으면 전용 함수 사용
        if (styleReferenceBase64) {
          generatedImages = await geminiService.generateStyleCopy(
            styleReferenceBase64,   // Image 1: 스타일 참조 (부모 Generation 이미지)
            sourceImageBase64,       // Image 2: 원본 제품 구조
            characterImageBase64,    // Image 3: 새 캐릭터
            v3Options,               // 추가 옵션 (viewpointLock, whiteBackground 등)
            options.outputCount ?? 2 // 생성할 이미지 수
          );
        } else {
          // 스타일 참조 이미지가 없으면 일반 IP 변경으로 대체
          console.warn(`⚠️ 스타일 참조 이미지 없음 - 일반 IP 변경으로 대체`);
